...
abstract contract FlashLoan {
    ...
    function takeFlashLoan(FlashLoanProviders flp, address token, uint256 amount) internal virtual {
        console.log("[Hack Progress] | Taking flashloan of %s %s from FlashLoanProviders[%s]", amount, token, uint256(flp));
        _flps.push(flp);
    ...
    function _fallback() internal virtual {
        console.log("[Hack Progress] | Execute attack");
        _executeAttack();
        if (_flps.length > 0) {
            FlashLoanProviders flp = currentFlashLoanProvider();
            if (flp.callbackFunctionSelector() == bytes4(msg.data[:4])) {
                console.log("[Hack Progress] | Attack completed successfully");
                _completeAttack();
                console.log("[Hack Progress] | Pay back flash loan");
                flp.payFlashLoan();
