...
library MakerDAOFlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        ...
        contextOne.dssFlash.flashLoan(address(this), token, amount, "");
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        (, address token, uint256 amount, uint256 fee, ) = unpackData(data);
        Context memory contextTwo = context();

        require(msg.sender == address(contextTwo.dssFlash), "MakerDAOFlashLoan: Callback msg.sender was not Maker DAO");
