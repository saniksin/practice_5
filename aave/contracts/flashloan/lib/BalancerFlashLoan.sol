...
library BalancerFlashLoan {
    ...
    function takeFlashLoan(address[] memory tokens, uint256[] memory amounts) internal {
        ...
        contextOne.balancerVault.flashLoan(address(this), tokens, amounts, "");
    }
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        ...
        contextTwo.balancerVault.flashLoan(address(this), tokens, amounts, "");
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        ...
        require(
            msg.sender == address(contextThree.balancerVault), "BalancerFlashLoan: Callback msg.sender was not the Balancer"
        );
        ...
        returns (address[] memory token, uint256[] memory amount, uint256[] memory feeAmount)
    {
        ...
        (address[] memory tokens, uint256[] memory amounts, uint256[] memory feeAmounts, ) =
