...
library UniswapV2FlashLoan {
    ...
    function takeFlashLoan(address[] memory tokens, uint256[] memory amounts) internal {
        ...
        Context memory contextOne = context(tokens);

        require(address(contextOne.uniswapV2Pair) != address(0), "UniswapV2Flashloan: Pair contract not found");

        address token0 = IUniswapV2Pair(contextOne.uniswapV2Pair).token0();
        ...
        IUniswapV2Pair(contextOne.uniswapV2Pair).swap(
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        ...
        tokens[0] = token;
        Context memory contextTwo = context(tokens);

        require(address(contextTwo.uniswapV2Pair) != address(0), "UniswapV2Flashloan: Pair contract not found");

        address token0 = IUniswapV2Pair(contextTwo.uniswapV2Pair).token0();
        //address token1 = IUniswapV2Pair(contextTwo.uniswapV2Pair).token1();
        ...
        IUniswapV2Pair(contextTwo.uniswapV2Pair).swap(
            // Uniswap V2 requires data be non empty for the flash loan callback to be called
            amount0,
            amount1,
            address(this),
    ...
    function payFlashLoan(bytes calldata data) internal {
        (, uint256 amount0, uint256 amount1, ) = unpackData(data);
