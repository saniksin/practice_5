...
library AAVEV1FlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        ...
        require(address(contextOne.AAVElendingPool) != address(0), "AAVEV1Flashloan: Query to AAVElendingPool failed.");

        contextOne.AAVElendingPool.flashLoan(address(this), token, amount, "");
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        ...
        (address asset, uint256 amount, uint256 fee, ) = unpackData(data);

        require(
            msg.sender == address(contextTwo.AAVElendingPool),
            "AAVEV1FlashLoan: Callback msg.sender was not the lending pool"
        );

        IERC20(asset).transfer(contextTwo.core, amount + fee);
    }
