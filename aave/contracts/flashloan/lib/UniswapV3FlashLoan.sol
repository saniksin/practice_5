...
library UniswapV3FlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        Context memory contextOne = context(address(0), token, amount);

        require(address(contextOne.uniswapV3Pool) != address(0), "UniswapV3Flashloan: Pair contract not found");

        address token0 = IUniswapV3Pool(contextOne.uniswapV3Pool).token0();
        //address token1 = IUniswapV3Pool(contextOne.uniswapV3Pool).token1();

        uint256 amount0;
        uint256 amount1;

        (amount0, amount1) = token0 == contextOne.asset ? (amount, amount1) : (amount0, amount);

        IUniswapV3Pool(contextOne.uniswapV3Pool).flash(
    ...
    function takeFlashLoan(address pair, address token, uint256 amount) internal {
        Context memory contextTwo = context(pair, token, amount);

        require(address(contextTwo.uniswapV3Pool) != address(0), "UniswapV3Flashloan: Pair contract not found");

        address token0 = IUniswapV3Pool(contextTwo.uniswapV3Pool).token0();
        //address token1 = IUniswapV3Pool(contextTwo.uniswapV3Pool).token1();

        uint256 amount0;
        uint256 amount1;

        (amount0, amount1) = token0 == contextTwo.asset ? (amount, amount1) : (amount0, amount);

        IUniswapV3Pool(contextTwo.uniswapV3Pool).flash(
    ...
    function payFlashLoan(bytes calldata data) internal {
        (uint256 fees0, uint256 fees1, ) = unpackData(data);
