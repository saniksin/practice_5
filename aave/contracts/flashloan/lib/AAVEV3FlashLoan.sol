...
library AAVEV3FlashLoan {
    ...
    function takeFlashLoan(address[] memory tokens, uint256[] memory amounts) internal {
        Context memory contextOne = context();
        require(address(contextOne.AAVEPool) != address(0), "AAVEV3Flashloan: Query to AAVElendingPool failed.");
        uint256[] memory modes = new uint256[](tokens.length);

        contextOne.AAVEPool.flashLoan(address(this), tokens, amounts, modes, address(this), "", 0);
    }
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        Context memory contextTwo = context();
        require(address(contextTwo.AAVEPool) != address(0), "AAVEV3Flashloan: Query to AAVElendingPool failed.");
        ...
        contextTwo.AAVEPool.flashLoan(address(this), tokens, amounts, modes, address(this), "", 0);
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        Context memory contextThree = context();
        (
        ...
        require(msg.sender == address(contextThree.AAVEPool), "AAVEV3FlashLoan: Callback msg.sender was not the pool");

        for (uint256 i = 0; i < assets.length; i++) {
            IERC20(assets[i]).approve(address(contextThree.AAVEPool), amounts[i] + premiums[i]);
        }
    }
