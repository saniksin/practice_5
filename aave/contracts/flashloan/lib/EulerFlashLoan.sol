...
library EulerFlashLoan {
    ...
    function takeFlashLoan(address token, uint256 amount) internal {
        Context memory contextOne = context(token);
        contextOne.dToken.flashLoan(amount, abi.encode(token, amount));
    }
    ...
    function payFlashLoan(bytes calldata data) internal {
        ...
        Context memory contextTwo = context(token);

        require(msg.sender == contextTwo.euler, "EulerFlashLoan: Callback msg.sender was not Euler");
